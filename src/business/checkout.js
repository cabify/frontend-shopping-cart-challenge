...
class Checkout {
  ...
  scan(aCode) {
    ...
    if (exists) this.scanItems[index].qty += 1;
  ...
  remove(aCode) {
    const index = this.scanItems.findIndex(scanItem => scanItem.code === aCode);
    const exists = index > -1;
    if (exists && !!this.scanItems[index].qty) this.scanItems[index].qty -= 1;
    return this;
  }
  ...
  getScans() {
    return this.scanItems.map((scan) => {
      const { price } = this.pricingRules.find(pr => pr.code === scan.code);
  ...
  scanCount() {
    return this.getScans().reduce((aux, scanItem) => aux + scanItem.qty, 0);
  }

  scanTotal() {
    return this.getScans().reduce((aux, scanItem) => aux + scanItem.total, 0);
  }
